...
export default function Preview() {
  ...
  function renderContent() {
    ...
    return (
      <>
        {viewAlbum?.type === '组合' ? (
          <div style={{ background: bgTo || '#fffdf1', borderRadius: 8, padding: 12, marginBottom: 0, flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: '#333', marginBottom: 12 }}>{giftMap[viewAlbum.albumId]?.name || albumMap[viewAlbum.albumId]?.name || (viewAlbum.productName && viewAlbum.productName !== '产品名称' ? viewAlbum.productName : null) || viewAlbum._albumData?.giftData?.name || viewAlbum._albumData?.productName || '产品名称'}</div>
            {viewAlbum.bannerUrl && (
              <div style={{ height: 'auto', aspectRatio: '2/1', background: `url(${viewAlbum.bannerUrl}) center/cover no-repeat`, borderRadius: 8, marginBottom: 16 }} />
            )}
          ...
          <div style={{ background: detailMode ? 'transparent' : bgTo || '#fffdf1', padding: detailMode ? 0 : 12, marginBottom: 0, flex: 1 }}>
            {(() => {
          ...
          <div style={{ background: bgTo || '#fffdf1', borderRadius: 8, padding: 12, marginBottom: 0, flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 0, display: 'none' }}>{currentCat.name}</div>
          ...
          <div style={{ background: bgTo || '#fffdf1', borderRadius: 8, padding: 12, marginBottom: 0, flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 0, display: 'none' }}>所有画册</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
              {allAlbums.map(a => (
  ...
  return (
    <>
      ...
      <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', maxWidth: '100vw', background: bgTo || '#fffdf1' }}>
        {!detailMode && bannerUrl && (
          <div style={{ flexShrink: 0, position: 'relative', zIndex: 10 }}>
            <div style={{ width: '100%', aspectRatio: '16/9', background: '#fff', position: 'relative' }}>
              ...
              {(bannerTitle || bannerSubtitle) && (
                <div style={{ position: 'absolute', bottom: -30, left: 0, width: '100%', height: 76 }}>
                  <svg viewBox="0 0 900 140" preserveAspectRatio="none" style={{ width: '100%', height: '100%', display: 'block' }}>
                    <defs>
                    ...
                    <path d="M28 0 H398 Q418 0 426 12 L482 68 Q502 80 510 80 H900 V140 H8 Q8 140 0 132 V28 Q0 0 28 0 Z" fill="url(#tabG)" filter="url(#tabS)" />
                  </svg>
                  <div style={{ position: 'absolute', left: 12, top: 0, bottom: 0, right: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', paddingTop: !detailMode && bannerUrl ? 30 : 0 }}>
          {!detailMode && (
            <div style={{ width: 'max-content', maxWidth: '40vw', background: bgTo || '#fffdf1', border: 'none', borderRadius: '0 20px 20px 0', overflowY: 'auto', flexShrink: 0 }}>
                  ...
                  categories.map(cat => (
                    <div key={cat.id} className="album-tree-group">
                      <div
                        ...
                        className={`album-tree-node album-tree-node-level1${selectedCat === cat.id ? ' active' : ''} album-tree-node-compact`}
