...
export default function Home() {
  ...
  async function generatePrompts(fest, count, refImageUrl, imgType, productInfo) {
    ...
    const base = (imgType === '详情图' || imgType === '白底图' || imgType === '样机图' || imgType === '场景图')
      ? (() => { const p = Array.from({ length: count }, () => ''); p[0] = imgType === '样机图' ? (promptCfg.prompt_mockup || '生成一个3d效果图，真实立体包装') : imgType === '场景图' ? (promptCfg.prompt_scene || '锁定原始产品主体，保持产品100%真实一致：严格保留产品原有的外观设计、产品结构、尺寸比例、颜色、材质纹理、零部件组合方式、品牌标识位置（如原图存在）。禁止改变产品形态，不重新设计，不增加或删除任何部件，不改变产品功能结构。将产品自然融入真实生活使用场景中，打造高级商业产品摄影效果。根据产品属性匹配合理环境：家居用品→温馨现代家庭空间，小家电→干净整洁的厨房客厅办公空间，电子产品→桌面办公居家娱乐科技生活场景，家具→高端室内空间自然光环境，日用品→日常生活使用环境。场景需要真实自然，符合产品实际使用逻辑，空间比例合理，光线符合真实摄影效果，产品与环境自然融合。生成超写实商业产品摄影图：2K高清分辨率，细节锐利清晰，专业摄影棚级画质，真实镜头景深效果，柔和自然光照，高级但真实的色彩表现，材质纹理清晰可见，产品边缘清晰立体，保留真实阴影和空间层次。产品作为画面视觉中心，保持居中或黄金比例构图，占据主要视觉区域，突出产品质感和功能特点，保持真实大小比例，不被环境遮挡。打造高端品牌广告摄影风格：干净自然的生活环境，简洁高级背景，温暖真实氛围，无杂乱物品，无过度装饰，无夸张特效。禁止：不改变产品颜色、结构、组合方式，不增加不存在的功能，不添加文字水印广告语，不生成虚假材质，不改变品牌Logo，不让场景抢占产品主体视觉。最终生成一张真实、高端、自然的商业产品场景摄影照片，类似品牌官网和电商详情页展示图') : (promptCfg.prompt_whitebg || '生成白底图'); return p })()
      : Array.from({ length: count }, () => '')
    setPrompts(base)
    try {
      const apiCount = (imgType === '样机图' || imgType === '白底图' || imgType === '场景图') ? 0 : (imgType === '详情图') ? count - 1 : count
      ...
      if (data.prompts) {
        const result = imgType === '样机图'
          ? Array.from({ length: count }, () => promptCfg.prompt_mockup || '生成一个3d效果图，真实立体包装')
          : imgType === '白底图'
            ? Array.from({ length: count }, () => promptCfg.prompt_whitebg || '生成白底图')
            : imgType === '场景图'
              ? Array.from({ length: count }, () => base[0] || promptCfg.prompt_scene || '')
              : imgType === '详情图'
              ? [promptCfg.prompt_whitebg || '生成白底图', ...data.prompts.slice(0, count - 1)]
