          ...
          {albumImages.map((a, i) => {
            ...
            return urls.map((url, j) => {
              ...
              return <div
                  ...
                  onClick={() => setPickerSelected(prev => { const n = new Set(prev); if (n.has(i)) n.delete(i); else n.add(i); return n })}
                >
                  <img src={url} alt="" style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', display: 'block' }} />
                  {urls.length > 1 && j === 0 && (
                    <div style={{ position: 'absolute', bottom: 4, right: 4, background: 'rgba(0,0,0,.55)', color: '#fff', fontSize: 10, padding: '1px 6px', borderRadius: 8 }}>{urls.length}张</div>
                  )}
                </div>
            })
          })}
